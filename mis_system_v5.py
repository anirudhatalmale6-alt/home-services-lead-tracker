...
for i in range(15):
    ...
    discount_pct = random.choice([0, 0, 0, 0.05, 0.10])  # Most no discount
    gst_rate = 0.18
    ...
    ws_income.cell(row=row_num, column=9).value = f"=G{row_num}*H{row_num}"  # Discount Amount
    ws_income.cell(row=row_num, column=10).value = f"=G{row_num}-I{row_num}"  # After Discount
    ws_income.cell(row=row_num, column=11).value = gst_rate
    ws_income.cell(row=row_num, column=12).value = f"=J{row_num}*K{row_num}"  # GST Amount
    ws_income.cell(row=row_num, column=13).value = f"=J{row_num}+L{row_num}"  # Total
...
# ONLY add dropdowns where needed (Service, Project Type, GST %, Payment Mode, Payment Status)
...
# FIXED: Only needed dropdowns - Category, GST %, Payment Mode, Payment Status
...
# NO dropdown on Notes column

# ============ EMPLOYEE SALARIES (FIXED - PF/ESI/HRA default 0) ============
...
for i, emp in enumerate(employees, start=2):
    ...
    ws_salary.cell(row=i, column=7).value = 0  # HRA default 0
    ws_salary.cell(row=i, column=8).value = 0  # Other Allowances default 0
    ws_salary.cell(row=i, column=9).value = f"=F{i}+G{i}+H{i}"  # Gross = Basic + HRA + Allowances
    ws_salary.cell(row=i, column=10).value = 0  # PF default 0
    ws_salary.cell(row=i, column=11).value = 0  # ESI default 0
    ws_salary.cell(row=i, column=12).value = 0  # Other deductions default 0
    ws_salary.cell(row=i, column=13).value = f"=J{i}+K{i}+L{i}"  # Total Deductions
    ws_salary.cell(row=i, column=14).value = f"=I{i}-M{i}"  # Net Salary
...
# FIXED: Correct dropdowns for contractor
...
# NO dropdown on Payment Date (column 11) - it's a date field

# ============ STOCK PURCHASES (FIXED - removed unnecessary dropdowns) ============
...
# FIXED: Only necessary dropdowns
...
# NO dropdowns on Vendor Name, Item Name, Quantity, Unit

# ============ MACHINES & EQUIPMENT (FIXED - added Invoice & GST columns) ============
...
# NO dropdowns on GST Amount, Total Amount columns

# ============ MACHINE MAINTENANCE (FIXED - added Status column, removed Notes dropdown) ============
...
# FIXED: Added Service Status dropdown, removed Notes dropdown
...
for i, cat in enumerate(machine_cats, start=5):
    ...
    ws_stock_rep.cell(row=i, column=2).value = f'=SUMIF(\'Machines & Equipment\'!D:D,A{i},\'Machines & Equipment\'!H:H)'
    ws_stock_rep.cell(row=i, column=3).value = f'=SUMIF(\'Machines & Equipment\'!D:D,A{i},\'Machines & Equipment\'!J:J)'
    ws_stock_rep.cell(row=i, column=4).value = f'=SUMIF(\'Machines & Equipment\'!D:D,A{i},\'Machines & Equipment\'!K:K)'
    for col in [2, 3, 4]:
...
ws_dash['A6'] = "=SUM('Income Tracker'!M:M)"
...
ws_dash['E6'] = "=SUM('Income Tracker'!J:J)-SUM('Expense Tracker'!F:F)-SUM('Employee Salaries'!N:N)-SUM('Contractor Payments'!J:J)-SUM('Stock Purchases'!K:K)-SUM('Machine Maintenance'!F:F)"
...
ws_dash['G6'] = "=IF(SUM('Income Tracker'!J:J)>0,E6/SUM('Income Tracker'!J:J),0)"
...
for i, svc in enumerate(services, start=19):
    ...
    ws_dash.cell(row=i, column=2).value = f'=SUMIF(\'Income Tracker\'!D:D,A{i},\'Income Tracker\'!M:M)'
...
# Save
wb.save('/var/lib/freelancer/projects/40182876/MIS_System_v5.xlsx')
print("MIS System v5 created successfully!")
