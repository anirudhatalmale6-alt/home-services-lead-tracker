...
metric_fill = PatternFill("solid", fgColor="E8F4FD")
...
# Metric Cards - Row 1
metric_labels = ["Total Revenue", "Total Expenses", "Net Profit", "Profit Margin"]
metric_formulas = [
    "=SUM('Income Tracker'!G:G)",
    "=SUM('Expense Tracker'!F:F)",
    "=B5-D5",
    "=IF(B5>0,F5/B5,0)"
]

ws_dash['A5'] = metric_labels[0]
ws_dash['B5'] = metric_formulas[0]
ws_dash['C5'] = metric_labels[1]
ws_dash['D5'] = metric_formulas[1]
ws_dash['E5'] = metric_labels[2]
ws_dash['F5'] = metric_formulas[2]
ws_dash['G5'] = metric_labels[3]
ws_dash['H5'] = metric_formulas[3]
...
ws_dash['H5'].alignment = center_align
ws_dash['F5'].font = Font(bold=True, size=14, color="008000")

# Metric Cards - Row 2
ws_dash['A7'] = "Active Machines"
ws_dash['B7'] = "=COUNTIF('Machines & Equipment'!G:G,\"Active\")"
ws_dash['C7'] = "Under Repair"
ws_dash['D7'] = "=COUNTIF('Machines & Equipment'!G:G,\"Under Repair\")"
ws_dash['E7'] = "Total Asset Value"
ws_dash['F7'] = "=SUM('Machines & Equipment'!F:F)"
ws_dash['G7'] = "Maintenance Cost"
ws_dash['H7'] = "=SUM('Machine Maintenance'!F:F)"

for col in ['A', 'C', 'E', 'G']:
    ws_dash[f'{col}7'].font = Font(bold=True, size=10, color="666666")
    ws_dash[f'{col}7'].alignment = center_align
for col in ['B', 'D']:
...
# Charts Data Section (Hidden data for charts)
...
ws_dash['B11'] = "Amount"
ws_dash['A12'] = "Revenue"
ws_dash['B12'] = "=SUM('Income Tracker'!G:G)"
ws_dash['A13'] = "Expenses"
ws_dash['B13'] = "=SUM('Expense Tracker'!F:F)"
ws_dash['B12'].number_format = currency_format
ws_dash['B13'].number_format = currency_format

# Service-wise Revenue Data
ws_dash['D11'] = "Service"
ws_dash['E11'] = "Revenue"
for i, svc in enumerate(services, start=12):
    ws_dash.cell(row=i, column=4).value = svc
    ws_dash.cell(row=i, column=5).value = f'=SUMIF(\'Income Tracker\'!D:D,D{i},\'Income Tracker\'!G:G)'
...
for i, cat in enumerate(expense_cats, start=12):
    ...
    ws_dash.cell(row=i, column=8).value = f'=SUMIF(\'Expense Tracker\'!C:C,G{i},\'Expense Tracker\'!F:F)'
...
# Create Service Revenue Pie Chart
chart2 = PieChart()
...
# Quick Stats Table
ws_dash['I38'] = "QUICK STATS"
ws_dash['I38'].font = Font(bold=True, size=12, color="FFFFFF")
ws_dash['I38'].fill = dash_header_fill
ws_dash.merge_cells('I38:L38')

stats_data = [
    ["Pending Payments (Income)", "=SUMIF('Income Tracker'!I:I,\"Pending\",'Income Tracker'!G:G)"],
    ["Pending Payments (Expenses)", "=SUMIF('Expense Tracker'!H:H,\"Pending\",'Expense Tracker'!F:F)"],
    ["Total Orders", "=COUNTA('Income Tracker'!B:B)-1"],
    ["Total Transactions", "=COUNTA('Stock Transactions'!B:B)-1"],
]

for i, (label, formula) in enumerate(stats_data, start=39):
    ws_dash.cell(row=i, column=9).value = label
    ws_dash.cell(row=i, column=10).value = formula
    ws_dash.cell(row=i, column=9).font = Font(bold=True)
    if "Pending" in label:
        ws_dash.cell(row=i, column=10).number_format = currency_format
        ws_dash.cell(row=i, column=10).font = Font(color="FF0000")
...
# Save the workbook
wb.save('/var/lib/freelancer/projects/40182876/MIS_System_v2.xlsx')
print("MIS System v2 with charts created successfully!")
