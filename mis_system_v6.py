...
ws_chem.auto_filter.ref = f"A1:L6"
...
ws_acc.auto_filter.ref = f"A1:K6"
...
# ============ STOCK TRANSACTIONS (FIXED - no dropdowns on Qty, Rate, Team) ============
...
ws_trans.auto_filter.ref = f"A1:M6"
...
dv_gst_trans.add('I2:I1000')
# NO dropdowns on Quantity (G), Unit Rate (H), Project/Team (L)
...
# Chemical Stock Summary
ws_stock_rep['A3'] = "CHEMICALS STOCK SUMMARY"
ws_stock_rep['A3'].font = Font(bold=True, size=12, color="008000")
ws_stock_rep.append(["Item ID", "Chemical Name", "Current Stock", "Min Level", "Status", "Est. Value (₹)"])
style_header(ws_stock_rep, 4, 6)

chem_items = [
    ("CH001", "Floor Cleaner", 70, 20, 120),
    ("CH002", "Glass Cleaner", 35, 15, 95),
    ("CH003", "Toilet Cleaner", 50, 20, 110),
    ("CH004", "Pesticide Spray", 35, 10, 350),
    ("CH005", "Surface Disinfectant", 35, 15, 180),
]
for i, (cid, name, stock, min_lvl, rate) in enumerate(chem_items, start=5):
    ws_stock_rep.cell(row=i, column=1).value = cid
    ws_stock_rep.cell(row=i, column=2).value = name
    ws_stock_rep.cell(row=i, column=3).value = stock
    ws_stock_rep.cell(row=i, column=4).value = min_lvl
    ws_stock_rep.cell(row=i, column=5).value = "OK" if stock >= min_lvl else "Low Stock"
    ws_stock_rep.cell(row=i, column=6).value = stock * rate
    ws_stock_rep.cell(row=i, column=6).number_format = currency_format

style_data_rows(ws_stock_rep, 5, 9, 6)
ws_stock_rep.cell(row=10, column=1).value = "TOTAL"
ws_stock_rep.cell(row=10, column=1).font = Font(bold=True)
ws_stock_rep.cell(row=10, column=6).value = "=SUM(F5:F9)"
ws_stock_rep.cell(row=10, column=6).font = Font(bold=True)
ws_stock_rep.cell(row=10, column=6).number_format = currency_format

# Accessories Stock Summary
ws_stock_rep['A13'] = "ACCESSORIES STOCK SUMMARY"
ws_stock_rep['A13'].font = Font(bold=True, size=12, color="0066CC")
ws_stock_rep.append(["Item ID", "Item Name", "Current Stock", "Min Level", "Status", "Est. Value (₹)"])
style_header(ws_stock_rep, 14, 6)

acc_items = [
    ("AC001", "Rubber Hose (10m)", 13, 5, 450),
    ("AC002", "Floor Mop", 25, 10, 250),
    ("AC003", "Microfiber Cloth", 70, 25, 80),
    ("AC004", "Rubber Gloves", 40, 15, 45),
    ("AC005", "Spray Bottles", 28, 10, 120),
]
for i, (aid, name, stock, min_lvl, rate) in enumerate(acc_items, start=15):
    ws_stock_rep.cell(row=i, column=1).value = aid
    ws_stock_rep.cell(row=i, column=2).value = name
    ws_stock_rep.cell(row=i, column=3).value = stock
    ws_stock_rep.cell(row=i, column=4).value = min_lvl
    ws_stock_rep.cell(row=i, column=5).value = "OK" if stock >= min_lvl else "Low Stock"
...
style_data_rows(ws_stock_rep, 15, 19, 6)
ws_stock_rep.cell(row=20, column=1).value = "TOTAL"
ws_stock_rep.cell(row=20, column=1).font = Font(bold=True)
ws_stock_rep.cell(row=20, column=6).value = "=SUM(F15:F19)"
ws_stock_rep.cell(row=20, column=6).font = Font(bold=True)
ws_stock_rep.cell(row=20, column=6).number_format = currency_format
...
ws_stock_rep['A32'].font = Font(bold=True, size=12, color="660066")
ws_stock_rep['A33'] = "Total Stock Purchases (Base)"
ws_stock_rep['B33'] = "=SUM('Stock Purchases'!K:K)"
ws_stock_rep['A34'] = "Total GST on Purchases"
ws_stock_rep['B34'] = "=SUM('Stock Purchases'!M:M)"
ws_stock_rep['A35'] = "Total with GST"
ws_stock_rep['B35'] = "=SUM('Stock Purchases'!N:N)"
ws_stock_rep['A36'] = "Paid"
ws_stock_rep['B36'] = "=SUMIF('Stock Purchases'!O:O,\"Paid\",'Stock Purchases'!N:N)"
ws_stock_rep['A37'] = "Pending"
ws_stock_rep['B37'] = "=SUMIF('Stock Purchases'!O:O,\"Pending\",'Stock Purchases'!N:N)"
ws_stock_rep['B37'].font = Font(color="FF0000")

for row in range(33, 38):
    ws_stock_rep.cell(row=row, column=2).number_format = currency_format

auto_width(ws_stock_rep, 6)
ws_stock_rep.column_dimensions['A'].width = 25
ws_stock_rep.column_dimensions['B'].width = 20
ws_stock_rep.auto_filter.ref = f"A4:F10"
...
# Metric Cards - Row 4-5
# Revenue
ws_dash['A4'] = "TOTAL REVENUE"
ws_dash['A4'].font = Font(bold=True, size=11, color="FFFFFF")
ws_dash['A4'].fill = PatternFill("solid", fgColor="28A745")
ws_dash['A4'].alignment = center_align
ws_dash.merge_cells('A4:B4')
ws_dash['A5'] = "=SUM('Income Tracker'!M:M)"
ws_dash['A5'].font = Font(bold=True, size=18)
ws_dash['A5'].number_format = currency_format
ws_dash['A5'].alignment = center_align
ws_dash.merge_cells('A5:B5')

# Expenses
ws_dash['C4'] = "TOTAL EXPENSES"
ws_dash['C4'].font = Font(bold=True, size=11, color="FFFFFF")
ws_dash['C4'].fill = PatternFill("solid", fgColor="DC3545")
ws_dash['C4'].alignment = center_align
ws_dash.merge_cells('C4:D4')
ws_dash['C5'] = "=SUM('Expense Tracker'!I:I)+SUM('Employee Salaries'!N:N)+SUM('Contractor Payments'!J:J)+SUM('Stock Purchases'!N:N)+SUM('Machine Maintenance'!I:I)"
ws_dash['C5'].font = Font(bold=True, size=18)
ws_dash['C5'].number_format = currency_format
ws_dash['C5'].alignment = center_align
ws_dash.merge_cells('C5:D5')

# Net Profit
ws_dash['E4'] = "NET PROFIT"
ws_dash['E4'].font = Font(bold=True, size=11, color="FFFFFF")
ws_dash['E4'].fill = PatternFill("solid", fgColor="007BFF")
ws_dash['E4'].alignment = center_align
ws_dash.merge_cells('E4:F4')
ws_dash['E5'] = "=SUM('Income Tracker'!J:J)-SUM('Expense Tracker'!F:F)-SUM('Employee Salaries'!N:N)-SUM('Contractor Payments'!J:J)-SUM('Stock Purchases'!K:K)-SUM('Machine Maintenance'!F:F)"
ws_dash['E5'].font = Font(bold=True, size=18, color="006600")
ws_dash['E5'].number_format = currency_format
ws_dash['E5'].alignment = center_align
ws_dash['E5'].fill = profit_fill
ws_dash.merge_cells('E5:F5')

# Profit Margin
ws_dash['G4'] = "PROFIT MARGIN"
ws_dash['G4'].font = Font(bold=True, size=11, color="FFFFFF")
ws_dash['G4'].fill = PatternFill("solid", fgColor="6F42C1")
ws_dash['G4'].alignment = center_align
ws_dash.merge_cells('G4:H4')
ws_dash['G5'] = "=IF(SUM('Income Tracker'!J:J)>0,E5/SUM('Income Tracker'!J:J),0)"
ws_dash['G5'].font = Font(bold=True, size=18)
...
# TAX & PAYROLL Section - Row 7
ws_dash['A7'] = "TAX SUMMARY"
ws_dash['A7'].font = Font(bold=True, size=12, color="FFFFFF")
ws_dash['A7'].fill = PatternFill("solid", fgColor="17A2B8")
ws_dash.merge_cells('A7:B7')

ws_dash['D7'] = "PAYROLL SUMMARY"
ws_dash['D7'].font = Font(bold=True, size=12, color="FFFFFF")
ws_dash['D7'].fill = PatternFill("solid", fgColor="FD7E14")
ws_dash.merge_cells('D7:E7')

ws_dash['G7'] = "PENDING ITEMS"
ws_dash['G7'].font = Font(bold=True, size=12, color="FFFFFF")
ws_dash['G7'].fill = PatternFill("solid", fgColor="E83E8C")
ws_dash.merge_cells('G7:H7')

# Tax Summary Data
tax_data = [
    ("GST Collected", "=SUM('Income Tracker'!L:L)"),
    ("GST Paid", "=SUM('Expense Tracker'!H:H)+SUM('Stock Purchases'!M:M)"),
    ("Net GST Payable", "=A9-A10"),
    ("TDS Deducted", "=SUM('Contractor Payments'!I:I)"),
]
for i, (label, formula) in enumerate(tax_data, start=8):
...
payroll_data = [
    ...
    ("Total Deductions", "=SUM('Employee Salaries'!M:M)"),
    ("Net Salary Paid", "=SUM('Employee Salaries'!N:N)"),
    ("Total Employees", "=COUNTA('Employee Salaries'!B:B)-1"),
]
for i, (label, formula) in enumerate(payroll_data, start=8):
    ws_dash.cell(row=i, column=4).value = label
    ws_dash.cell(row=i, column=5).value = formula
    if "Employees" not in label:
...
pending_data = [
    ...
    ("Pending GST", "=SUMIF('GST Invoices'!I:I,\"Pending\",'GST Invoices'!G:G)"),
    ("TDS Pending", "=SUM('Contractor Payments'!I:I)-SUMIF('Contractor Payments'!N:N,\"Yes\",'Contractor Payments'!I:I)"),
]
for i, (label, formula) in enumerate(pending_data, start=8):
    ws_dash.cell(row=i, column=7).value = label
    ws_dash.cell(row=i, column=8).value = formula
    ws_dash.cell(row=i, column=8).number_format = currency_format
    ws_dash.cell(row=i, column=8).font = Font(color="FF0000")
...
# Save
wb.save('/var/lib/freelancer/projects/40182876/MIS_System_v6.xlsx')
print("MIS System v6 created successfully!")
